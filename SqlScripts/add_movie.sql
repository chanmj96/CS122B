...
BEGIN
    ...
    THEN
        ...
        SELECT CONCAT("The movie was not found. Adding new movie to database with id=",new_mid);
    ELSE
    ...
    THEN
        SELECT CONCAT("The star was not found. Inserting star into table with id=",new_sid);
        INSERT INTO stars(id,name,birthYear) VALUES (new_sid,Astar_name, NULL);
    ELSE 
        SET new_sid = star_number;
    END IF;

    SELECT CONCAT("Adding stars_in_movies relation (",new_sid,", ",new_mid,")");
    ...
    THEN
        SELECT "The genre was not found. Inserting genre into table with id="+new_gid;
        INSERT INTO genres(id,name) VALUES (new_gid,Agenre_name);
    ELSE
        SET new_gid = genre_number;
    END IF; 
    SELECT CONCAT("Adding genres_in_movies relation (",new_gid,", ",new_mid,")");
    INSERT INTO genres_in_movies(genreId,movieId) VALUES (new_gid,new_mid);
END
$$

DELIMITER ;
CALL add_movie("TestMovie", 2018, "Matt", "C", "Genre");
